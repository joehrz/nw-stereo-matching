...
{
    for (int d = startDiag; d <= endDiag; d++) {
        int i_start = max(1, d - (cols - 1));
        int i_end   = min(d, rows - 1);
        int numElements = i_end - i_start + 1;
        
        for (int idx = threadIdx.x; idx < numElements; idx += blockDim.x) {
            int i = i_end - idx;   // Mapping in descending order
            int j = d + 1 - i;     // Ensures i + j = d + 1
